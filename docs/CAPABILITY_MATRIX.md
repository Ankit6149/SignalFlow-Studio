...
| Owner Golden Path 1 manual thought → authentic approval | Implemented for owner use through both the advanced/manual controls and the explicit high-confidence `Prepare for review` path: manual Signal → Opportunity + NarrativeMemory repetition gate → confidence gates → provenance-tagged angle/strategy decisions → ContentPiece → immutable LinkedIn/X revisions → separate evidence/authenticity checks → Today → exact owner approve/request-change/reject. Exact approval creates `prepared_internal` NarrativeMemory; Feedback/StyleMemory learning remains pending | #166/#206/#208 |
