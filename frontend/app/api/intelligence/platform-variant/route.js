...
export async function POST(request) {
  ...
  try {
    ...
    if (task.taskType !== INFERENCE_TASK_TYPES.PLATFORM_VARIANT) {
      return json({ ok: false, code: "unsupported_inference_task", error: "This endpoint only accepts platform_variant tasks." }, 400);
    }
    const input = normalizePlatformVariantTaskInput(body?.input || {});
    if (input.strategy.workspaceId !== task.workspaceId
      || input.contentPiece.workspaceId !== task.workspaceId
      || input.variant.workspaceId !== task.workspaceId
      || input.sourceSignal.workspaceId !== task.workspaceId
      || input.identityContext.workspaceId !== task.workspaceId) {
      return json({ ok: false, code: "cross_workspace_inference", error: "Platform generation inputs must all belong to the task workspace." }, 403);
    }
    if (input.dataClassification !== task.dataClassification || input.sourceSignal.privacyClassification !== task.dataClassification) {
      return json({ ok: false, code: "inference_privacy_mismatch", error: "Platform generation classification must match the canonical source Signal." }, 400);
    }
    ...
    const raw = await generateJSON({
      provider: providerId,
      prompt: buildPlatformVariantPrompt(input),
      modelOverride: model || null,
      config: {
        allowServerKey: isOwner,
        maxTokens: input.variant.destination === "x" ? 1800 : 2600,
      },
    });
    const output = acceptPlatformVariantDraft(raw, input.variant.destination);
    ...
    return json({
      ok: false,
      code: error?.code || "platform_variant_generation_error",
      error: error?.message || "SignalFlow could not generate this platform draft.",
    }, status);
  }
}
