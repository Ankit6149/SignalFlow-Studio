...
test("stdio server enforces lifecycle, initializes, and lists SignalFlow tools", async (t) => {
  ...
  assert.equal(toolList.result.tools.length, 9);
  ...
  assert.equal(toolList.result.tools[5].name, "signalflow_start_campaign");
  assert.equal(toolList.result.tools[8].name, "signalflow_create_campaign");
