...
function firstLineHeadline(value) {
  ...
  return first.length <= 220 ? first : `${first.slice(0, 217).trimEnd()}…`;
}
...
export default function SignalsWorkspace() {
  ...
  const reload = useCallback(async () => {
    try {
      setSignals(await application.listSignals({ includeArchived: true }));
    } catch (error) {
  ...
  async function submitSignal(event) {
    ...
    setMessage(null);
    try {
      const created = await application.createManualSignal({
      ...
      setMessage({
        type: "success",
        text: "Signal saved in this browser. Nothing was generated or published. You can evaluate it when you want editorial judgment.",
      });
      if (created?.signalId) router.prefetch(`/signals/${created.signalId}`);
    } catch (error) {
  ...
  async function saveEdit(signalId) {
    ...
    await runAction(() => application.updateSignalMetadata(signalId, {
    ...
    <main className={styles.page} id="main-content">
      ...
      <section className={styles.hero} aria-labelledby="signals-title">
        <div>
          ...
          <p>A signal is a durable record of a thought, lesson, milestone, release, question, or event. Saving one does not create a campaign and does not call AI. When you choose Evaluate, opportunity judgment becomes a separate persisted step.</p>
        </div>
        <div className={styles.principleCard}>
          <span>OWNER-FIRST CONTRACT</span>
          <strong>Your job is judgment.</strong>
          <p>SignalFlow keeps the context and now carries a saved signal into an explicit editorial decision before narrative or platform copy exists.</p>
        </div>
      </section>
      ...
      {message && <div className={`${styles.message} ${message.type === "error" ? styles.messageError : styles.messageSuccess}`} role={message.type === "error" ? "alert" : "status"} aria-live="polite">{message.text}</div>}
      ...
      <div className={styles.workspaceGrid}>
        <section className={styles.captureCard} aria-labelledby="capture-signal-title">
          ...
          <form onSubmit={submitSignal} className={styles.form}>
            ...
            <label className={styles.fullField}><span>Boundary note <em>optional</em></span><textarea value={form.boundaryNote} onChange={(event) => updateForm("boundaryNote", event.target.value)} placeholder="Do not mention the client name or expose private repository details." rows={3} maxLength={4000} /></label>
            <div className={styles.formFooter}>
              <div><strong>Stored locally in this browser</strong><span>Cloud sync and automatic event detection are not claimed here.</span></div>
        ...
        <aside className={styles.nextCard} aria-labelledby="next-title">
          <span className={styles.nextTag}>GOLDEN PATH · NOW IN BUILD</span>
          <h2 id="next-title">Signals stay evidence until you ask for judgment.</h2>
          <ol>
            <li><b>01</b><span>Capture and persist the signal.</span></li>
            <li><b>02</b><span>Evaluate why it may matter, evidence quality, repetition, boundaries, and possible angles.</span></li>
            <li><b>03</b><span>Select one angle—or Something else—before LinkedIn/X copy exists.</span></li>
          </ol>
          <p>Automatic detection is still not implemented. The owner deliberately starts evaluation from a saved signal.</p>
        </aside>
      </div>
      ...
      <section className={styles.librarySection} aria-labelledby="saved-signals-title">
        <div className={styles.libraryHeader}>
          ...
          <label className={styles.filterLabel}><span>Show</span><select value={filter} onChange={(event) => setFilter(event.target.value)}><option value="active">Active</option><option value="snoozed">Snoozed</option><option value="ignored">Ignored</option><option value="archived">Archived</option><option value="all">All</option></select></label>
        </div>
                ...
                <article className={styles.signalCard} key={signal.signalId}>
                  ...
                  {isEditing ? (
                    <div className={styles.editForm}>
                      ...
                      <div className={styles.editActions}><button type="button" onClick={() => saveEdit(signal.signalId)} disabled={busy}>Save changes</button><button type="button" className={styles.quietButton} onClick={() => { setEditingId(null); setEditDraft(null); }} disabled={busy}>Cancel</button></div>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className={styles.signalFoot}>
                        ...
                        <div className={styles.signalActions}>
                          {!["archived", "ignored"].includes(signal.status) && <button type="button" onClick={() => router.push(`/signals/${signal.signalId}`)} disabled={busy}>Evaluate</button>}
                          ...
                          {["ignored", "archived", "snoozed"].includes(signal.status) ? (
                            ...
                            <><button type="button" onClick={() => snooze24Hours(signal.signalId)} disabled={busy}>Snooze 24h</button><button type="button" onClick={() => runAction(() => application.ignoreSignal(signal.signalId), "Signal ignored. It remains in history.")} disabled={busy}>Ignore</button></>
                          )}
