...
import {
  acceptAuthenticityCritic,
  acceptEvidenceCritic,
  buildAuthenticityCriticPrompt,
  buildEvidenceCriticPrompt,
  normalizeVariantCriticInput,
} from "../../../../lib/ai/platformVariantCritics.mjs";
import {
...
export async function POST(request) {
  ...
  try {
    ...
    if (!CRITIC_TASKS.has(task.taskType)) {
      return json({ ok: false, code: "unsupported_inference_task", error: "This endpoint accepts only evidence_critique or authenticity_critique tasks." }, 400);
    }
    const input = normalizeVariantCriticInput(body?.input || {});
    ...
    if (workspaceValues.some((value) => value !== task.workspaceId)) {
      return json({ ok: false, code: "cross_workspace_inference", error: "Critic inputs must all belong to the task workspace." }, 403);
    }
    ...
    if (!selected) {
      return json({ ok: false, code: "inference_route_unavailable", error: "No configured operational model route is available for draft review." }, 503);
    }
    ...
    const evidenceTask = task.taskType === INFERENCE_TASK_TYPES.EVIDENCE_CRITIQUE;
    const prompt = evidenceTask ? buildEvidenceCriticPrompt(input) : buildAuthenticityCriticPrompt(input);
    ...
    return json({
      ...
      provenance: {
        ...
        promptVersion: evidenceTask ? "evidence_critic_v1" : "authenticity_critic_v1",
        reviewedAt,
      },
    });
  } catch (error) {
    const status = error?.code === "inference_privacy_route_denied" ? 403 : 500;
    return json({ ok: false, code: error?.code || "platform_variant_critic_error", error: error?.message || "SignalFlow could not review this draft." }, status);
  }
}
