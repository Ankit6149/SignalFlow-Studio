...
| Owner Golden Path 1 manual thought → authentic approval | Implemented browser-local for owner use through both advanced/manual controls and the explicit high-confidence `Prepare for review` path: manual Signal → Opportunity + NarrativeMemory repetition gate → confidence gates → provenance-tagged angle/strategy decisions → ContentPiece → immutable LinkedIn/X revisions → bounded StyleMemory generation context → separate evidence/authenticity checks → Today → exact owner approve/request-change/reject. Exact approval creates `prepared_internal` NarrativeMemory and eligible owner corrections feed explainable StyleMemory. Final issue closure still requires the complete sanitized owner acceptance/reopen evidence required by #166 | #154/#166/#206/#208 |
